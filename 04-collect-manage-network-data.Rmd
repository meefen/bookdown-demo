...
I also want to mention that research ethics is never a static topic at all. In many emerging research spaces (e.g., the Internet), research ethics remains highly debatable [@Rivers2014-wt;@Kraut2004-se]. Feel free to share your thoughts via Hypo by adding the `ethics` hashtag, or in the Slack `general` channel.
...
To summarize, this page provides a basic overview of how SNA data could be structured. You may encounter different ways of representing SNA data, such as a relationship matrix with rows and columns representing the same set of actors. Such representations could all be derived from a *tidy* dataset discussed above. In data collection, we will strive for keeping as much raw information as possible (such as timestamp), to enable analyses that only come to your mind afterwards.
...
Below are R codes to get you started (also downloadable from [the #SNAEd Github repository](https://github.com/meefen/sna-ed/blob/master/assets/lab_1.R)). Please make sure the `igraph` R package is installed in your system. You can go far beyond the codes. And make sure you share what you make to our Slack `assignment` channel.
...
To begin, download the [dataset here](https://raw.githubusercontent.com/meefen/sna-ed/master/assets/lesmis/lesmis.csv). Open it using a spreadsheet software and see what's in there. But make sure you never change anything in the file.
