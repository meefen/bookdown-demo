...
## Collecting Ego-Centric Network Data {#ego-data}
...
Below I demonstrate the difference between one-step vs. two-step ego networks using the *Les Miserables* dataset[^1]. Which type of ego networks would make more sense for analyzing characters in this novel?
...
2. On Slack, **share** your thoughts on these two questions (listed below) that are raised in Sections \@ref(ego-intro) and \@ref(ego-data). **Discuss** among each other by offering comments, expanding on each other's ideas, identifying limitations, etc. 
...
### Lab 3(c): Analyzing Ego-Centric Networks
...
In `igraph`, there are several functions you can use to extract information about ego networks. Run `?ego` to learn more.[^2] Please add the following lines to your Lab 3, and tinker with different parameters. 
